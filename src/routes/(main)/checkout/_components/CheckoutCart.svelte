...
<script lang="ts">
	...
	let {
			cart, subtotal, shippingFee, tax, discountAmount, total,
			coupon, onApplyCoupon, onRemoveCoupon, nextStep
		} = $props<{
	...
	<div class="lg:col-span-1">
		...
		<div class="bg-white rounded-lg border border-gray-200 p-6 mb-4">
			...
			<div class="space-y-2 text-sm">
				...
				{/if}
				<div class="flex justify-between text-gray-600"><span>Shipping Fee</span><span>₱{shippingFee.toLocaleString()}</span></div>
				<div class="flex justify-between text-gray-600"><span>Tax (12% VAT)</span><span>₱{tax.toLocaleString()}</span></div>
			...
			<button
				onclick={nextStep}
				class="w-full mt-6 px-6 py-3 bg-primary text-white font-medium rounded-full hover:bg-primary-700 transition"
			>
		...
		<div class="bg-white rounded-lg border border-gray-200 p-4">
			...
			{#if coupon.applied}
				<div class="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-md">
					...
					<button onclick={onRemoveCoupon} class="text-red-600 text-sm hover:underline">Remove</button>
				</div>
			{:else}
