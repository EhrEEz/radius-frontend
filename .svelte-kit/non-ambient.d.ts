...
declare module "$app/types" {
	export interface AppTypes {
		RouteId(): "/(main)" | "/(auth)" | "/" | "/(main)/checkout" | "/(main)/checkout/_components" | "/(auth)/forgot-password" | "/(auth)/login" | "/(main)/products" | "/(main)/products/[slug]" | "/(auth)/reset-password" | "/(auth)/signup" | "/(auth)/verify-email";
		RouteParams(): {
		...
		Pathname(): "/" | "/checkout" | "/forgot-password" | "/login" | "/products" | `/products/${string}` & {} | "/reset-password" | "/signup" | "/verify-email";
